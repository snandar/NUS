...
function ПеренаправитьНаНашПроигрыватель(сКодКанала)
{
	...
	//
	// Firefox 52, 59+: location.replace(moz-extension://) не замещает, а добавляет адрес в историю.
	// location.replace(moz-extension://) и location.assign(moz-extension://) дают загружаемой странице полные права.
	// Firefox 53..58: location.replace(moz-extension://) работает правильно.
	// location.replace(moz-extension://) и location.assign(moz-extension://) дают загружаемой странице права content script.
	// https://bugzilla.mozilla.org/show_bug.cgi?id=1436321
	// https://bugzilla.mozilla.org/show_bug.cgi?id=1389833
	// HACK Передаем смену адреса в фоновую страницу, чтобы дать проигрывателю полные права, как в Chrome и Edge,
	// и заменить адрес вместо добавления.
	//
	if (ВЕРСИЯ_ДВИЖКА_БРАУЗЕРА >= 57 && !ЭТО_ПЛАНШЕТ)
	{
		chrome.runtime.sendMessage(
		{
			сЗапрос: 'ЗапуститьПроигрыватель',
...
{
	// Во время обновления расширения Firefox делает следующее:
	// 1. Удаляет песочницу со старой версией content script. Пока нет простого способа определить этот момент.
	// 2. Изменения DOM, включая добавленные <script>, остаются.
	// 3. Добавляет песочницу с новой версией content script. Если расширение удалено, то этот шаг пропускается.
	// Эта функция выполняет действия, которые в идеале должны выполняться на шаге 2.
	// Также см. ОТКЛАДЫВАТЬ_СОХРАНЕНИЕ_НА в common.js.
	// https://bugzilla.mozilla.org/show_bug.cgi?id=1223425
	// https://bugs.chromium.org/p/chromium/issues/detail?id=414213
	const сузУдалить = document.getElementsByClassName('tw5-js-удалить');
	while (сузУдалить.length !== 0)
	{
		м_Журнал.Окак(`[Twitch 5] Удаляю ${сузУдалить[0].nodeName} старой версии`);
		сузУдалить[0].remove();
	}
