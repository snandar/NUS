...
(() =>
{
	...
	{
		// Firefox 52: Во время загрузки страницы можно получить событие,
		// обращение к свойствам которого кидает исключение.
		try {оСобытиеМыши.bubbles} catch (_) {return}
	...
	if (window.TouchEvent)
	{
		...
		const оАктивный = {capture: true, passive: false};
		const оПассивный = {capture: true, passive: true};
		// Firefox 61 Android: Нужно отменить touchstart и touchend, чтобы не показывать контекстное меню браузера.
		// Если не отменить touchend, то элемент останется :active.
